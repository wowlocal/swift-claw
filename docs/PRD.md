...
- **FR-S3.** *(v1)* **Idempotency, split honestly into two mechanisms** (full detail in [`ARCHITECTURE.md` §7]):
  ...
  - **(b) External side effects (outbound replies):** delivered via a **transactional outbox** — intent committed → effect attempted → completion recorded. Definite pre-send failures remain retryable; `mayHaveBeenSent` outcomes are quarantined for operator review so a restart loop cannot spam the same reply. Each reply chunk is its own outbox step so a partial multipart send recovers.
...
- **FR-C4.** Audit events for job create/execute/cancel/failure. Proactive runs have their own daily spend budget, including learning calls when enabled.
- **FR-C5.** *(P-learning)* Scheduled learning is off by default behind `CLAW_LEARNING_ENABLED`. One authenticated owner correction or two compatible negative runs sharing an issue code can produce a bounded lesson candidate. Trials expose at most three runs; two positive runs may promote, and one negative ends the trial. The owner can inspect, edit, approve, reject, roll back a current promotion, or confirm an epoch reset through `/learning`. Lessons remain untrusted data and cannot change job configuration or tool policy. The fixed `scheduled-learning/v1` algorithm has no per-job overrides; [`ARCHITECTURE.md` §14](ARCHITECTURE.md#14-scheduler-architecture-inc-4) defines the shipped contracts.
...
### 6.10 Operator UX
- **FR-O1.** *(commands land with their features)* Telegram command menu: at minimum `/start`, `/help`, `/status`, `/new`, `/stop`, `/cost` in v1; `/memory`, `/retry` in v1; `/model`, `/approve`, `/reject`, `/schedule`, `/cancel`, `/skills`, and `/learning` as their features land.
...
- **NFR-Reliability.** Retries only on retryable errors with capped exponential backoff + full jitter at exactly one layer with a retry budget; transactional outbound delivery retries only when the prior attempt is known not to have reached Telegram and quarantines ambiguous outcomes (FR-S3); crash-recoverable explicit run state machine with a boot reconciliation sweep; graceful shutdown; never silently loop (hard budgets stop and ask). Deterministic startup failures (bad config, missing secret, `409`, `SQLITE_FULL`) **back off rather than hot-loop**; disk-full refuses new turns and replies "storage full" once without crash-looping; `doctor` does a free-disk preflight.
...
| **P-learning** | FR-C5 | Opt-in generic scheduled learning: authenticated feedback, bounded candidate trials, exact promotion and rollback, `/learning` inspection/reset, restart-safe pinned context and reference-aware retention. |
